...
export const yeshivaTypeEnum = pgEnum("yeshiva_type", [
  "kollel",
  "yeshiva",
]);


export const YeshivaDetails = createTable(
  "yeshiva_details",
  {
    id: bigserial({ mode: "number" }).primaryKey(),
    beneficiaryNationalId: varchar("beneficiary_national_id", { length: 10 }).notNull().unique().references(() => Person.nationalId, { onDelete: "restrict" }),
    ...
    yeshivaCertificateUploadId: ulid("upload").references(() => Upload.id, { onDelete: "set null" }),
